-- Migration 25: Fix end day streak and use END_DAY for streak increments
...
BEGIN
    ...
    CASE p_action_type
        ...
        WHEN 'END_DAY' THEN
            -- Idempotency check: if already awarded today, skip duplicate calculation
        ...
        WHEN 'START_DAY' THEN
            -- Idempotency check: if started today, don't recount
            IF v_profile.last_day_started_at::DATE = p_today THEN
    ...
    -- 4. Streak logic (only for EVENING_RITUAL or END_DAY)
    v_new_streak := v_profile.current_streak;
    IF p_action_type IN ('EVENING_RITUAL', 'END_DAY') AND (v_profile.last_active_date IS NULL OR v_profile.last_active_date != p_today) THEN
    ...
    UPDATE profiles SET
        ...
        last_active_date = CASE WHEN p_action_type IN ('EVENING_RITUAL', 'END_DAY', 'MORNING_RITUAL') THEN p_today ELSE last_active_date END, -- active date reflects structured work
    ...
    ON CONFLICT (user_id, date) DO UPDATE SET
        ...
        updated_at = EXCLUDED.day_ended_at -- Use the incoming timestamp to trigger update timestamp
    ;
