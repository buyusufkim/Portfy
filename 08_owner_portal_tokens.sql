...
DO $$ 
BEGIN
    IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE policyname = 'Allow user to view their own tokens') THEN
...
DO $$ 
BEGIN
    IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE policyname = 'Allow user to insert tokens') THEN
...
DO $$ 
BEGIN
    IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE policyname = 'Allow user to revoke tokens') THEN
...
CREATE INDEX IF NOT EXISTS idx_owner_portal_tokens_token ON owner_portal_tokens (token);
CREATE INDEX IF NOT EXISTS idx_owner_portal_tokens_property_id ON owner_portal_tokens (property_id);
CREATE INDEX IF NOT EXISTS idx_owner_portal_tokens_expires_at ON owner_portal_tokens (expires_at);
