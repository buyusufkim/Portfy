...
-- 7. Gün Sonu Kapanış ve (9) Sabah 10 Dakika Planı
...
ADD COLUMN IF NOT EXISTS unsold_reason TEXT;

-- 8 & 13. Lead Sessizleşme ve Müşteri Unutma Koruması
ALTER TABLE leads 
ADD COLUMN IF NOT EXISTS last_contacted_at TIMESTAMPTZ;

-- 10. "Aradın mı?" Takip Disiplini
ALTER TABLE tasks 
ADD COLUMN IF NOT EXISTS requires_call BOOLEAN DEFAULT false;

-- (Ek Güvence: contacts tablosu yoksa veya ileride açılırsa diye placeholder, şu an lead'ler CRM çekirdeği)
-- ALTER TABLE contacts ADD COLUMN IF NOT EXISTS last_contacted_at TIMESTAMPTZ;

-- Güvenlik Politikaları (RLS)
ALTER TABLE content_calendar ENABLE ROW LEVEL SECURITY;
CREATE POLICY IF NOT EXISTS "Users can manage their own content_calendar" ON content_calendar FOR ALL USING (auth.uid() = user_id);

ALTER TABLE micro_goals ENABLE ROW LEVEL SECURITY;
CREATE POLICY IF NOT EXISTS "Users can manage their own micro_goals" ON micro_goals FOR ALL USING (auth.uid() = user_id);

ALTER TABLE territory_plans ENABLE ROW LEVEL SECURITY;
CREATE POLICY IF NOT EXISTS "Users can manage their own territory_plans" ON territory_plans FOR ALL USING (auth.uid() = user_id);

ALTER TABLE referrals ENABLE ROW LEVEL SECURITY;
CREATE POLICY IF NOT EXISTS "Users can manage their own referrals" ON referrals FOR ALL USING (auth.uid() = user_id);

ALTER TABLE user_activations ENABLE ROW LEVEL SECURITY;
CREATE POLICY IF NOT EXISTS "Users can manage their own user_activations" ON user_activations FOR ALL USING (auth.uid() = user_id);

ALTER TABLE portal_traffic_logs ENABLE ROW LEVEL SECURITY;
CREATE POLICY IF NOT EXISTS "Users can read their own portal_traffic_logs" ON portal_traffic_logs FOR SELECT USING (auth.uid() = user_id);

ALTER TABLE daily_rituals ENABLE ROW LEVEL SECURITY;
CREATE POLICY IF NOT EXISTS "Users can manage their own daily_rituals" ON daily_rituals FOR ALL USING (auth.uid() = user_id);

ALTER TABLE weekly_reports ENABLE ROW LEVEL SECURITY;
CREATE POLICY IF NOT EXISTS "Users can manage their own weekly_reports" ON weekly_reports FOR ALL USING (auth.uid() = user_id);
