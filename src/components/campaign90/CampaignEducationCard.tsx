                            ...
                            <div className="space-y-4">
                                {curriculum.mini_quiz.map((q, idx) => (
                                    <div key={idx} className="space-y-2">
                                        <p className="text-sm font-bold text-slate-700">{idx + 1}. {q.question}</p>
                                        <div className="flex flex-col gap-1.5 pl-4">
                                            {q.options.map((opt, oIdx) => (
                                                <label key={oIdx} className="flex items-start gap-2 text-[13px] text-slate-600 font-medium cursor-pointer hover:text-slate-900">
                                                    <input type="radio" name={`quiz-${idx}`} className="mt-0.5" />
                                                    <span>{opt}</span>
                                                </label>
                                            ))}
                                        </div>
