...
export const DashboardView: React.FC<DashboardViewProps> = ({
  ...
  const [dashboardTab, setDashboardTab] = useState<"action" | "revenue">(
    "action",
  );
  ...
  const bolgemItems: TopActionItem[] = (tasks || [])
    .filter(
      (t) =>
        t.source === "bolgem" &&
        !t.completed &&
    ...
    .map((task) => {
      let name = "Bölge Teması";
      let cat = "Belirtilmedi";
      let rel = "Belirtilmedi";
      let pot = "Belirtilmedi";

      if (task.metadata) {
        if (typeof task.metadata === "object" && task.metadata !== null) {
          const md = task.metadata as Record<string, unknown>;
          if (md.contact_name) name = String(md.contact_name);
          else if (md.region_pin_id) name = String(md.region_pin_id);

          if (md.category) cat = String(md.category);
          if (md.relationship_level) rel = String(md.relationship_level);
          if (md.potential) pot = String(md.potential);
        }
      }

      return {
        type: "bolgem_followup" as const,
        originalItem: task,
        id: `bolgem-${task.id}`,
        title: `${name} tekrar temas bekliyor`,
        subtitle: `${cat} • ${rel} • Potansiyel: ${pot}`,
        desc: task.title,
        icon: MapPin,
        colorClass: "text-emerald-600 bg-emerald-100",
        ringClass: "hover:ring-emerald-200",
      };
    })
    .slice(0, 3);
        ...
        <div className="flex items-center gap-3">
          <div className="text-right hidden sm:block">
            ...
            <div className="text-[10px] font-bold text-orange-600 uppercase tracking-wider">
              {gamifiedStats?.level_name || "Broker"}
            </div>
          </div>
          <div
      ...
      <section className="grid grid-cols-2 gap-4">
        <Card className="p-4 md:p-6 !bg-orange-600 !text-white border-none shadow-lg shadow-orange-200">
          <div className="flex justify-between items-start">
            ...
            >
              {gamifiedStats?.level_name || "..."}
            </Badge>
          </div>
          <div className="mt-4">
            <div className="text-2xl font-bold text-white">
              {gamifiedStats ? (
                (gamifiedStats.points || 0).toLocaleString()
              ) : (
                <Skeleton className="h-8 w-20 bg-white/20" />
              )}
            </div>
            <div className="text-white font-bold text-[10px] uppercase tracking-wider mt-1">
              Toplam Puan
            </div>
          </div>
        </Card>
        ...
        <Card className="p-4 md:p-6">
          ...
          <div className="mt-4">
            <div className="text-2xl font-bold text-slate-900">
              {gamifiedStats ? (
                `${gamifiedStats.streak || 0} Gün`
              ) : (
                <Skeleton className="h-8 w-16" />
              )}
            </div>
            <div className="text-slate-600 text-[10px] uppercase font-bold tracking-wider mt-1">
              Peş Peşe Seri
            </div>
          </div>
        </Card>
          ...
          <div className="relative z-10 flex items-center gap-4 md:gap-6">
            <div className="relative w-16 h-16 shrink-0 md:w-20 md:h-20">
              ...
              <div className="absolute inset-0 flex items-center justify-center flex-col">
                ...
                <span className="text-[7px] md:text-[8px] uppercase font-bold text-slate-400">
                  Momentum
                </span>
              </div>
            </div>
              ...
              <div className="relative">
                {canUseAiCoach ? (
                  ...
                  <div className="text-sm font-bold leading-relaxed text-white/30 italic blur-[3px] select-none line-clamp-2">
                    Yapay zeka asistanınız bugün kime odaklanmanız gerektiğini
                    analiz etti. Görmek için tıklayın...
                  </div>
                )}
                    ...
                    <div className="flex-1">
                      <div className="flex items-center gap-2">
                        <h3 className="text-lg font-bold text-orange-900">
                          Sabah 10 Dakika Planı
                        </h3>
                        <div className="text-[10px] font-bold text-orange-600 bg-orange-100 px-2 py-0.5 rounded uppercase">
                          Ritüel
                        </div>
                      </div>
                      <p className="text-xs text-orange-800/70 font-medium mt-1 mb-4 leading-relaxed">
                        Güne başlamadan mikro hedefini belirle. Bugünü kazanmak
                        için tek ve en kritik odak noktanı yaz.
                      </p>
                      ...
                      <button
                        onClick={() => {
                          if (!microGoalInput.trim()) {
                            toast.error(
                              "Güne başlamak için mikro hedefini yaz.",
                            );
                            return;
                          }
                        ...
                        disabled={
                          !microGoalInput.trim() ||
                          addMicroGoalMutation.isPending
                        }
              ...
              <>
                  ...
                  <div className="flex items-center justify-between px-1">
                    <div>
                      ...
                      <p className="text-xs text-slate-500 font-medium">
                        Odaklanman gereken en önemli işler
                      </p>
                    </div>
                  </div>
                  ...
                  {visiblePriorities.length > 0 ? (
                    <div className="grid grid-cols-1 gap-3">
                      {visiblePriorities.map((item) => (
                        <Card
                          key={item.id}
                          className={`p-4 bg-white border border-slate-100 shadow-sm transition-all flex flex-col gap-3 group hover:ring-1 hover:shadow-md ${item.ringClass}`}
                        >
                          <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-3">
                            <div className="flex items-start gap-3 flex-1 min-w-0">
                              <div
                                className={`w-10 h-10 rounded-xl flex items-center justify-center shrink-0 mt-0.5 ${item.colorClass}`}
                              >
                                <item.icon size={20} />
                              </div>
                            ...
                            <div className="flex shrink-0 w-full sm:w-auto mt-1 sm:mt-0 justify-end">
                                ...
                                <button
                                  onClick={() =>
                                    ...
                                    completeTaskMutation.mutate({
                                      task: item.originalItem,
                                    })
                                  }
                                  className="w-full sm:w-auto whitespace-nowrap bg-indigo-50 hover:bg-indigo-100 text-indigo-600 px-3 py-1.5 rounded-lg text-[11px] font-bold transition-colors shadow-sm text-center"
                                >
                              ...
                              {item.type === "daily" && (
                                <button
                                  ...
                                  className="w-full sm:w-auto whitespace-nowrap bg-blue-50 hover:bg-blue-100 text-blue-600 px-3 py-1.5 rounded-lg text-[11px] font-bold transition-colors shadow-sm text-center"
                                >
                                  Tamamla
                                </button>
                              )}
                              {item.type === "drip" && (
                                <button
                                  onClick={() => {
                                    api
                                      ...
                                      .then(() => {
                                        ...
                                        toast.success(
                                          "Takip görevi başarıyla tamamlandı!",
                                        );
                                      })
                                      .catch(() =>
                                        toast.error(
                                  ...
                                  className="w-full sm:w-auto whitespace-nowrap bg-orange-50 hover:bg-orange-100 text-orange-600 px-3 py-1.5 rounded-lg text-[11px] font-bold transition-colors shadow-sm text-center"
                                >
                                  İletildi
                                </button>
                              )}
                              {item.type === "alert" && (
                                <button
                                  onClick={() => {
                                    if (
                                      setSelectedLead &&
                                      item.originalItem.lead &&
                                      item.originalItem.lead.id
                                    ) {
                                      setSelectedLead(
                                        item.originalItem.lead as Lead,
                                      );
                                    }
                                    setActiveTab("crm");
                                  }}
                                  className="w-full sm:w-auto whitespace-nowrap bg-red-50 hover:bg-red-100 text-red-600 px-3 py-1.5 rounded-lg text-[11px] font-bold transition-colors shadow-sm text-center"
                                >
                                  İncele
                                </button>
                              )}
                              {item.type === "smart_rec" && (
                                <button
                                  ...
                                  className="w-full sm:w-auto whitespace-nowrap bg-amber-50 hover:bg-amber-100 text-amber-600 px-3 py-1.5 rounded-lg text-[11px] font-bold transition-colors shadow-sm text-center"
                                >
                                  Düzenle
                                </button>
                              )}
                              {item.type === "bolgem_followup" && (
                                <div className="flex gap-2 shrink-0 w-full sm:w-auto">
                                  <button
                                    ...
                                    className="flex-1 sm:flex-none whitespace-nowrap bg-indigo-50 hover:bg-indigo-100 text-indigo-600 px-3 py-1.5 rounded-lg text-[11px] font-bold transition-colors shadow-sm text-center"
                                  >
                                    Bölgem'de Aç
                                  </button>
                                  <button
                                    onClick={() => {
                                      api
                                        ...
                                        .then(() => {
                                          ...
                                          toast.success(
                                            "Bölge takibi işaretlendi!",
                                          );
                                        })
                                        .catch(() => toast.error("Hata oluştu"));
                                    }}
                                    className="flex-1 sm:flex-none whitespace-nowrap bg-emerald-50 hover:bg-emerald-100 text-emerald-600 px-3 py-1.5 rounded-lg text-[11px] font-bold transition-colors shadow-sm text-center"
                                  >
                                    Tamamlandı
                                  </button>
                                </div>
                      ...
                      {todaysPriorities.length > 5 && (
                        <div className="flex gap-2 pt-1">
                          {todaysPriorities.length > visiblePriorityCount && (
                            <button
                              onClick={() =>
                                setVisiblePriorityCount((prev) =>
                                  Math.min(prev + 5, todaysPriorities.length),
                                )
                              }
                ...
                {(activeMicroGoal || microGoalInput.trim()) && (
                  <div className="bg-indigo-50 border border-indigo-100 p-4 rounded-3xl flex items-center gap-4 mt-4 mb-2">
                    ...
                    <div>
                      <h3 className="text-xs font-bold text-indigo-900 uppercase tracking-widest mb-0.5">
                        MİKRO HEDEF
                      </h3>
                ...
                {dailyPlan && (
                  <div className="grid grid-cols-3 gap-3">
                    ...
                    <div className="bg-indigo-50 border border-indigo-100 rounded-2xl p-3 text-center">
                      ...
                      <div className="text-lg font-black text-indigo-900">
                        {dailyPlan.completed_followups}/
                        {dailyPlan.planned_followups}
                      </div>
                    </div>
                    <div className="bg-indigo-50 border border-indigo-100 rounded-2xl p-3 text-center">
                      <div className="text-[10px] font-bold text-slate-500 uppercase tracking-wider mb-1">
                      ...
                      <div className="text-lg font-black text-indigo-900">
                        {dailyPlan.completed_portfolio_actions}/
                        {dailyPlan.planned_portfolio_actions}
                      </div>
                    </div>
                  </div>
                )}
                  ...
                  <section className="pt-2">
                    <div className="flex items-center justify-between mb-3 px-1">
                      <div>
                        ...
                        <p className="text-[10px] text-slate-500">
                          Bugün için aklında kalması gereken kısa notlar.
                        </p>
                      </div>
                      {setActiveTab && (
                        <button
                          onClick={() => setActiveTab("tasks")}
                          className="text-[10px] font-bold text-indigo-600 hover:text-indigo-700 bg-indigo-50 px-2.5 py-1 rounded-lg transition-colors"
                        >
                          Tümünü Gör
                        </button>
                      )}
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                      {flowNotes.map((note) => (
                        <Card
                          key={note.id}
                          className="p-3 bg-white border border-slate-100 flex flex-col justify-between hover:shadow-md transition-shadow"
                        >
                          <div>
                            <div className="flex items-center justify-between mb-2 gap-2">
                              ...
                              <span
                                className={`text-[9px] font-bold px-1.5 py-0.5 rounded shrink-0 ${
                                  note.sourceLabel === "Kişisel"
                                    ? "bg-orange-50 text-orange-600"
                                    : note.sourceLabel === "İçerik"
                                      ? "bg-fuchsia-50 text-fuchsia-600"
                                      : note.sourceLabel === "CRM"
                                        ? "bg-blue-50 text-blue-600"
                                        : note.sourceLabel === "Bölgem"
                                          ? "bg-yellow-50 text-yellow-600"
                                          : note.sourceLabel === "Portföy"
                ...
                {(() => {
                  ...
                  const isLowProgress =
                    (gamifiedStats?.daily_progress || 0) < 40;
                  const canRescue =
                    isLowProgress && currentHour >= 16 && !rescueSession;
                    ...
                    <div className="relative z-10 flex items-center gap-4 md:gap-6">
                      ...
                      <div className="text-left">
                        <div className="flex items-center gap-2">
                          ...
                          <Badge
                            variant="info"
                            className="!bg-slate-800 !text-slate-400 border-none text-[10px]"
                          >
                            {todayStr}
                          </Badge>
                        </div>
            ...
            {revenueLoading && !revenueStats ? (
              ...
              <Card className="p-8 text-center bg-white">
                ...
                <p className="text-slate-500 text-sm mt-2">
                  Satış ve gelirlerinizi takip etmek için portföy ve CRM
                  kullanmaya başlayın.
                </p>
              </Card>
            )}
