...
export const DashboardView: React.FC<DashboardViewProps> = ({
  ...
  const getAlertPriority = (typeStr: string) => {
  ...
  const getAlertCategory = (typeStr: string) => {
  ...
  const normalizeLeadName = (name?: string) => {
    if (!name) return "";
    return name.trim().toLowerCase().replace(/\s+/g, " ");
  };

  const dedupedAlerts = Object.values((leadAlerts || []).reduce((acc: Record<string, LeadAlert>, alert) => {
