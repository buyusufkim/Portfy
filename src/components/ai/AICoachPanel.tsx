...
export const AICoachPanel: React.FC = () => {
  ...
  if (!insight) {
    return (
      <div className="p-10 text-center bg-slate-50 border border-slate-100 rounded-[40px] space-y-6">
        <div className="w-20 h-20 bg-orange-100 rounded-3xl flex items-center justify-center text-orange-600 mx-auto">
          <Sparkles size={40} />
        </div>
        <div className="space-y-2">
          <h4 className="text-xl font-bold text-slate-900">AI Koç Hazır</h4>
          <p className="text-sm text-slate-500 font-medium max-w-sm mx-auto">
            Bugünkü verilerini analiz edip sana net aksiyon planı çıkarır.
          </p>
        </div>
        <button
          onClick={() => refetch()}
          className="px-6 py-3 bg-orange-600 hover:bg-orange-700 text-white rounded-xl text-sm font-bold flex items-center justify-center gap-2 mx-auto transition-colors"
        >
          <Sparkles size={16} /> AI Koçu Çalıştır
        </button>
      </div>
    );
  }
          ...
          {insight?.is_rescue_mode_recommended && (
            <div className="bg-orange-600/20 border border-orange-600/30 p-4 rounded-2xl flex items-center gap-3">
              ...
              <p className="text-xs font-bold text-orange-100">
                Rescue Mode Öneriliyor: Günü kurtarmak için hala vaktin var!
              </p>
            </div>
          )}
        </div>
...
const CoachActionCard: React.FC<{
  ...
  return (
    <motion.div
      ...
      className={`p-6 rounded-[32px] border ${colors[action.priority] || colors.medium} flex flex-col justify-between gap-6 group hover:shadow-lg transition-all`}
    >
      <div className="space-y-4">
        <div className="flex justify-between items-start">
          <div
            className={`w-12 h-12 rounded-2xl flex items-center justify-center ${action.priority === "high" ? "bg-orange-100" : "bg-white"} shadow-sm`}
          >
            {icons[action.type] || <Sparkles size={20} />}
          </div>
